...
Ray Camera::generate_ray(double x, double y) const { 
  // sensor plance represent your screen
  ...
  Vector3D rd = Vector3D(
    bottom_left.x +(top_right.x-bottom_left.x)*x,
    bottom_left.y + (top_right.y-bottom_left.y)*y,
    -1.0
  );

  rd = c2w *rd;
  rd.normalize();
  // calling the constructor of ray
  Ray ray = Ray(Vector3D(pos), Vector3D(rd),fClip ,0 );
  ray.min_t= nClip; 
  return ray;
