...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  // function in runinng for one pixel
   
  Vector2D origion  = Vector2D(x,y);
  int num_samples = ns_aa;
  Spectrum output = Spectrum(0,0,0);
  
  if (num_samples==1){
    double cx = (x+0.5)/sampleBuffer.w; // normalizing   adding 0.5 to center the location of ray
    double cy = (y+0.5)/sampleBuffer.h; // normalizing
    Ray temp =  camera->generate_ray(cx,cy);
    temp.depth = max_ray_depth; //how many times ray bounce 
    output = trace_ray(temp ,true);

  }else{

    for(int i=0;i<ns_aa;i++){
        Vector2D rand = gridSampler->get_sample(); //random offset
        double  cx = (x+rand.x)/sampleBuffer.w;
        double  cy = (y+rand.y)/sampleBuffer.h;
        Ray temp = camera->generate_ray(cx,cy);
        temp.depth = max_ray_depth;
        output += trace_ray(temp, true);
    }
    output /= num_samples; //averge
  }
...
void PathTracer::raytrace_tile(int tile_x, int tile_y,
  ...
  size_t ns_aa_tile = tile_samples[tile_idx_x + tile_idx_y * num_tiles_w];
