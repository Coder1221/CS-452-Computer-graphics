...
void DrawRend::rasterize_line( float x0, float y0,
  ...
  // resetting jump in according to the slope
  if (abs(m) > 1) {
    dx = x1==x0 ? 0 : 1/abs(m);
    dy = x1==x0 ? (y1-y0)/abs(y1-y0) : m/abs(m);
  }

  // The main loop
  while (floor(x) <= floor(x1) && abs(y-y0) <= abs(y1-y0)) {
...
bool DrawRend::inside_tri(float x0, float y0, float x1, float y1, float x2, float y2, float xp, float yp) {
  // check if xp and yp are inside the triangle with vertices x0, y0, x1, y1, x2, y2
  
  float dot_product1 = dot_product(x0,y0,x1,y1,xp,yp);
  float dot_product2 = dot_product(x1,y1,x2,y2,xp,yp);
  float dot_product3 = dot_product(x2,y2,x0,y0,xp,yp);
    	
  // for both clockwise and anticlockwise orderings of triangle vertices
  return (dot_product1 >= 0 && dot_product2 >= 0 && dot_product3 >= 0) || (dot_product1 < 0 && dot_product2 < 0 && dot_product3 < 0);
}

// Part 3: use tri->color as explained
void DrawRend::rasterize_triangle( float x0, float y0,
  ...
  float max_y = max(y0, max(y1,y2));
	...
	float up = 1.0/samples_per_side;

  //Loop going through the required range of pixels
  for(int i=min_x; i<max_x; i++) {
    for(int j=min_y; j<max_y; j++) {

      //Loop through all the sub-pixels in a pixel/SampleBuffer   
      for(int sr=0; sr<samples_per_side; sr++) {
        for(int sc=0; sc<samples_per_side; sc++) {

          //absolute subpixel row, column coordinates
          float xp = i+((sr+0.5)*up); 
          float yp = j+((sc+0.5)*up);
	
          if(inside_tri(x0,y0,x1,y1,x2,y2,xp,yp)) //check if xp and yp are inside triangle of given vertices 0-1-2
          { 
            ...
            if(tri!=NULL){
              ...
              // SampleParams temp;
              // temp.psm=1;
              SampleParams temp = {Vector2D(0, 0), Vector2D(0, 0), Vector2D(0, 0), psm, lsm};
              color = tri->color(x0,y0,x1,y1,x2,y2,xp,yp,temp);
              samplebuffer[j][i].fill_color(sr,sc,color);
