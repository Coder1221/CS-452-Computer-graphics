 ...
 */
 ...
 Color TexTri::color(float x0, float y0,float x1, float y1,float x2, float y2, float px, float py, SampleParams sp) {
    ...
    // the following variables contain the uv coords at the 3 vertices of the triangle in action.

    p0_uv; p1_uv; p2_uv; // index into texture coordinates

    Vector3D bary = barycentric(x0,y0,x1,y1,x2,y2,px,py);

    //Save the final value in this variable before you return.
    sp.p_uv = p0_uv*bary[0] + p1_uv*bary[1] + p2_uv*bary[2];
