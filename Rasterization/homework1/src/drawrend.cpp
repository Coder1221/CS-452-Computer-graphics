...
void DrawRend::rasterize_fig( float x0, float y0,float x1, float y1,float x2, float y2, Color color, Triangle *tri) {
  if(!toggle_scanline) 
    rasterize_triangle(x0,y0,x1,y1,x2,y2,color,tri);  
  else 
    rasterize_scanline(x0,y0,x1,y1,x2,y2,color,tri);

}
  
void DrawRend::rasterize_scanline( float x0, float y0,float x11, float y1,float x22, float y2,Color color, Triangle *tri) {
...
void DrawRend::rasterize_point( float x, float y, Color color ) {
  ...
  samplebuffer[row][col].fill_pixel(color);
  return;
}
...
void DrawRend::rasterize_line( float x0, float y0, float x1, float y1, Color color) {
  // DDA algorithum
 // https://www.youtube.com/watch?v=W5P8GlaEOSI  // ye tu abdul bari ha  ye tu accha baccha ha xD
  ...
  float m = _deltay / _deltax;
  auto steps = max(abs(_deltay) ,abs(_deltax) );

  auto  dx = _deltax/steps;
  auto  dy = _deltay/steps;

  // cout<<x0<<"(start)"<<y0<<endl;
  // cout<<x1<<"(target)"<<y1<<endl;
  // cout<<m<<"(slope)"<<endl;
  // cout<<"------------------"<<endl;
  
  for(auto i=0;i<=steps;i++){
    rasterize_point(tempx,tempy,color);
    tempy+=dy;
    tempx+=dx;
  }
...
bool DrawRend::inside(float x0, float y0, float x1, float y1,float x2, float y2 ,float px ,float py){
  
  float dot1 = dot_product(x0,y0,x1,y1,px,py);
  float dot2 = dot_product(x1,y1,x2,y2,px,py);
  float dot3 = dot_product(x2,y2,x0,y0,px,py);
...
void DrawRend::rasterize_triangle( float x0, float y0, float x1, float y1,float x2, float y2,Color color, Triangle *tri) {
...
//Part7: Modify your loop to fill every sub_pixel inside the trianlges.
  ...
  for(float i = floor(x_min);i<ceil(x_max);i+=0.5){
    for (float j=floor(y_min); j<ceil(y_max); j+=0.5){
        // subpixels supersamplings     
        // quadrants in one pixel (i,j)
        int sub = sqrt(sample_rate);
        for(int m=0;m<(int)sqrt(sample_rate);m++){
          for(int k=0;k<(int)sqrt(sample_rate);k++){              
              
              float x = (float)i + (float)(m+0.5)/(float)sub;
              float y = (float)j + (float)(k+0.5)/(float)sub;

              if(inside(x0,y0,x1,y1,x2,y2,x,y)){
                samplebuffer[j][i].fill_color(k,m,color);
              }
          }
        }
    }
