...
void DrawRend::rasterize_point( float x, float y, Color color ) {
  ...
  samplebuffer[y1][x1].fill_pixel(color);
  return;
}
...
float DrawRend::dot_product(float x0, float y0,float x1, float y1, float xp, float yp){
  ...
  return ((-1*(xp-x0)*(y1-y0)) + (yp-y0)*(x1-x0));
}
...
//Part7: Modify your loop to fill every sub_pixel inside the trianlges.
  ...
  auto x_max = max(x0 ,max(x1,x2));
  auto y_max = max(y0 ,max(y1,y2));
  auto x_min = min(x0, min(x1,x2));
  auto y_min = min(y0, min(y1,y2));

  for (auto i=x_min; i<x_max; i+=0.5){
    for (auto j=y_min;j<y_max; j+=0.5){
      float dot1 = dot_product(x0,y0,x1,y1,i+0.5,j+0.5);
      float dot2 = dot_product(x1,y1,x2,y2,i+0.5,j+0.5);
      float dot3 = dot_product(x2,y2,x0,y0,i+0.5,j+0.5);
